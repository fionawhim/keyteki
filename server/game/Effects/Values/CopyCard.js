 ...
 * appropriate places.
 *
 * **WARNING:** This does not un-listen any reactions or persistent effects from
 * the underlying card when replacing it with a copy. This relies on checks such
 * as in TriggeredAbility’s eventHandler to ignore reactions that are not
 * currently in the card’s `reactions` list (which is sensitive to )
 *
 * As of PV/CC, this problem does not come up, because:
 ...
 *   Sidekick) the hand, so the card’s underlying 'in play' abilities don’t get
 *   registered in the first place.
 * - Gĕzdrutyŏ the Arcane, the only card that _becomes_ a token creature from
 *   being in play, only has an action, not persistent effects.
